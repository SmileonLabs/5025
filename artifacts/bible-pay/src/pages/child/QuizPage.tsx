...
const MOCK_QUIZZES: Record<string, QuizQuestion[]> = {
  default: [
    {
      question: "하나님께서 세상을 만드실 때 첫째 날 만드신 것은 무엇인가요?",
      options: ["🌊 바다", "💡 빛", "🌿 풀과 나무", "⭐ 별"],
      correctIndex: 1,
    },
    {
      question: "하나님이 세상을 다 만드시고 마지막 날에 하신 일은 무엇인가요?",
      options: ["🐟 물고기를 만드셨어요", "🌈 무지개를 만드셨어요", "😴 쉬셨어요", "🏔️ 산을 만드셨어요"],
      correctIndex: 2,
    },
  ],
  "시편 23편": [
    {
      question: "시편 23편에서 하나님을 무엇에 비유하나요?",
      options: ["👑 왕", "🐑 목자", "🌊 강", "🦅 독수리"],
      correctIndex: 1,
    },
    {
      question: "시편 23편에서 목자는 나를 어디로 인도하나요?",
      options: ["🏔️ 높은 산으로", "🌲 푸른 초장으로", "🌊 깊은 바다로", "🌆 큰 도시로"],
      correctIndex: 1,
    },
  ],
  "요한복음 3:16": [
    {
      question: "요한복음 3장 16절에서 하나님은 세상을 어떻게 하셨나요?",
      options: ["만드셨어요", "사랑하셨어요", "심판하셨어요", "잊으셨어요"],
      correctIndex: 1,
    },
    {
      question: "하나님이 독생자를 주신 이유는 무엇인가요?",
      options: [
        "세상을 아름답게 하려고",
        "우리가 멸망하지 않고 영생을 얻게 하려고",
        "천사들을 보내려고",
        "성전을 짓게 하려고",
      ],
      correctIndex: 1,
...
export default function QuizPage() {
  ...
  const { selectedChildId, children, missions, rewardChild } = useAppContext();
    ...
    setTimeout(() => {
      if (currentQ + 1 < questions.length) {
        ...
        if (allCorrect) {
          ...
          if (child) {
            const success = rewardChild(child.id, reward, `${passage} 퀴즈 완료 🎉`);
            if (!success) {
                ...
                <div>
                  ...
                  <button
                    ...
                    className={`w-full p-4 rounded-[20px] border-2 text-left transition-all flex items-center gap-3 ${
                      selectedBook === mission.title && !customPassage
                  ...
                  {BIBLE_BOOKS.slice(0, 12).map(book => (
                    <button
                      ...
                      className={`px-3 py-2 rounded-full text-sm font-bold transition-all ${
                        selectedBook === book && !customPassage
                  ...
                  {questions.map((_, i) => (
                    <div
                      ...
                      className={`w-8 h-2 rounded-full transition-all ${
                        i < currentQ ? "bg-primary" : i === currentQ ? "bg-primary/40" : "bg-gray-200"
                      }`}
                    />
                  ))}
                </div>
                ...
                {questions[currentQ].options.map((option, idx) => {
                  ...
                  if (showFeedback) {
                    if (idx === questions[currentQ].correctIndex) {
                      btnStyle = "border-2 border-green-400 bg-green-50 text-green-800";
                    } else if (idx === selectedAnswer) {
                      btnStyle = "border-2 border-red-300 bg-red-50 text-red-700";
                    }
                  ...
                  return (
                    ...
                    >
                      ...
                      <span>{option}</span>
                      {showFeedback && idx === questions[currentQ].correctIndex && (
                        <CheckCircle2 className="w-5 h-5 text-green-500 ml-auto shrink-0" />
                      )}
                      {showFeedback && idx === selectedAnswer && idx !== questions[currentQ].correctIndex && (
                  ...
                  <div className="text-center">
                    ...
                    >
                      <p className="text-white font-black text-lg">
                        용돈이 쏙 들어왔어요! +{reward.toLocaleString("ko-KR")}원 💰
                      </p>
                    </motion.div>
                  </div>
                  <div className="flex items-center gap-2 bg-white rounded-[20px] px-5 py-4 shadow-sm border border-gray-100 w-full">
                    ...
                    <p className="text-sm font-bold text-gray-700">
                      ...
                      <span className="text-primary-foreground font-black">
                        ₩{children.find(c => c.id === child.id)?.balance.toLocaleString("ko-KR")}
                      </span>
                    </p>
                  </div>
                </>
                ...
                <>
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-gray-500">
                      {answeredCorrectly.filter(Boolean).length}문제 맞혔어요.
                      <br />다시 한번 도전해볼까요?
                    </p>
                  </div>
                </>
              ...
              <div className="flex flex-col gap-3 w-full mt-4">
                {!allCorrect && (
                  <Button
                    ...
                    className="w-full h-[52px] rounded-[14px] font-bold bg-primary hover:bg-primary/90 text-white"
                ...
                <Button
                  ...
                  variant="outline"
                  className="w-full h-[52px] rounded-[14px] font-bold border-gray-200"
